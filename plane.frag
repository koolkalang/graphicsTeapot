...
void main(void){
	...
	float shininess = 1;
	tColor = vec4(texture(tex, gl_TexCoord[0].st));
	...
	for(int i = 0; i < TOTAL_LIGHTS; i++){
	    ...
	    //vec4 diffuse_color = (1 *vec4(tColor*textureWeight, 0.0)) * max(dot(N,L), 0.0);
...
//	gl_FragColor = enviroColor;
	gl_FragColor = diffuse_sum;
//	if(diffuse_sum.a > .75)
//		gl_FragColor = vec4(diffuse_sum.r,diffuse_sum.g,diffuse_sum.b, diffuse_sum.a*0);
//	else

		gl_FragColor = vec4(diffuse_sum.r*1.5,diffuse_sum.g*1.5,diffuse_sum.b*1.5, diffuse_sum.a);
